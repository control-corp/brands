...
namespace Micro\Cache\Backend;

use Micro\Cache\Cache;

class File extends BackendAbstract implements BackendExtendedInterface
{
    /**
     * Available options
     *
    ...
    public function __construct(array $options = array())
    {
        ...
        if (isset($this->_options['file_name_prefix'])) { // particular case for this option
            if (!preg_match('~^[a-zA-Z0-9_]+$~D', $this->_options['file_name_prefix'])) {
                Cache::throwException('Invalid file_name_prefix : must use only [a-zA-Z0-9_]');
            }
        }
        if ($this->_options['metadatas_array_max_size'] < 10) {
            Cache::throwException('Invalid metadatas_array_max_size, must be > 10');
        }
    ...
    public function setCacheDir($value, $trailingSeparator = true)
    {
        if (!is_dir($value)) {
            ...
            if (!is_dir($value)) {
                Cache::throwException(sprintf('cache_dir "%s" must be a directory', $value));
            }
        }

        if (!is_writable($value)) {
            Cache::throwException(sprintf('cache_dir "%s" is not writable', $value));
        }
     ...
     * Available modes are :
     *
     * \Micro\Cache::CLEANING_MODE_ALL (default)    => remove all cache entries ($tags is not used)
     * \Micro\Cache::CLEANING_MODE_OLD              => remove too old cache entries ($tags is not used)
     * \Micro\Cache::CLEANING_MODE_MATCHING_TAG     => remove cache entries matching all given tags
     *                                               ($tags can be an array of strings or a single string)
     * \Micro\Cache::CLEANING_MODE_NOT_MATCHING_TAG => remove cache entries not {matching one of the given tags}
     *                                               ($tags can be an array of strings or a single string)
     * \Micro\Cache::CLEANING_MODE_MATCHING_ANY_TAG => remove cache entries matching any given tags
     *                                               ($tags can be an array of strings or a single string)
     *
    ...
    public function getFillingPercentage()
    {
        ...
        if ($total == 0) {
            Cache::throwException('can\'t get disk_total_space');
        } else {
            if ($free >= $total) {
                return 100;
            }
            ...
            if (is_file($file)) {
                ...
                switch ($mode) {
                    ...
                    case Cache::CLEANING_MODE_MATCHING_ANY_TAG:
                        ...
                        break;
                    default:
                        Cache::throwException('Invalid mode for clean() method');
                        break;
                }
            }
            ...
            if (is_file($file)) {
                ...
                switch ($mode) {
                    ...
                    case 'matchingAny':
                        ...
                        break;
                    default:
                        Cache::throwException('Invalid mode for _get() method');
                        break;
                }
            }
            if ((is_dir($file)) and ($this->_options['hashed_directory_level']>0)) {
