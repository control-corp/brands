...
abstract class Statement implements Statement\StatementInterface
{
    /**
    ...
    /**
     * @var \Micro\Database\Profiler\Query
     */
     ...
     * @param mixed $sql Either a string or \Micro\Database\Select.
     */
        ...
        foreach ($this->_sqlSplit as $key => $val) {
            if ($val == '?') {
                if ($this->_adapter->supportsParameters('positional') === false) {
                    throw new Statement\Exception("Invalid bind-variable position '$val'");
                }
            } else if ($val[0] == ':') {
                if ($this->_adapter->supportsParameters('named') === false) {
                    throw new Statement\Exception("Invalid bind-variable name '$val'");
                }
            }
            $this->_sqlParam[] = $val;
        }
    ...
    {
        $data = array();
        if ($style === Database::FETCH_COLUMN && $col === null) {
            $col = 0;
        }
        if ($col === null) {
    ...
    public function fetchColumn($col = 0)
    {
        $data = array();
        $col = (int) $col;
        $row = $this->fetch(Database::FETCH_NUM);
        if (!is_array($row)) {
            return false;
        }
        return $row[$col];
    ...
    public function fetchObject($class = 'stdClass', array $config = array())
    {
        ...
        $row = $this->fetch(Database::FETCH_ASSOC);
        if (!is_array($row)) {
            return false;
        }
     ...
     * @return bool
     * @throws \Micro\Database\Statement\Exception
     */
    public function setFetchMode($mode)
    {
        switch ($mode) {
            ...
            default:
                $this->closeCursor();
                throw new Statement\Exception('invalid fetch mode');
                break;
        }
    }
     ...
     * particular \Micro\Database\Statement object.
     *
