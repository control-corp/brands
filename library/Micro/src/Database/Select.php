...
class Select
{
     ...
     * Class constructor
     *
     * @param \Micro\Database\Adapter\AdapterAbstract $adapter
     */
    public function __construct(\Micro\Database\Adapter\AdapterAbstract $adapter)
    {
     ...
     * table.
     *
     * The second parameter can be a single string or \Micro\Database\Expr object,
     * or else an array of strings or \Micro\Database\Expr objects.
     *
     * The first parameter can be null or an empty string, in which case
     * no correlation name is generated or prepended to the columns named
     * in the second parameter.
     *
     * @param  array|string|\Micro\Database\Expr $name The table name or an associative array
     ...
     *
     * The parameter can be a single string or \Micro\Database\Expr object,
     * or else an array of strings or \Micro\Database\Expr objects.
     *
     * @param  array|string|\Micro\Database\Expr $cols The columns to select from this table.
    ...
    public function columns($cols = '*', $correlationName = null)
    {
        ...
        if (!array_key_exists($correlationName, $this->_parts[self::FROM])) {
            throw new \Micro\Database\Select\Exception("No table has been specified for the FROM clause");
        }
     ...
     *
     * The first parameter has to be an array of \Micro\Database\Select or
     * sql query strings.
     *
     * <code>
     * $sql1 = $db->select();
    ...
    {
        if (!is_array($select)) {
            throw new \Micro\Database\Select\Exception('union() only accepts an array of \Micro\Database\Select instances of sql query strings.');
        }

        if (!in_array($type, self::$_unionTypes)) {
            throw new \Micro\Database\Select\Exception("Invalid union type '{$type}'");
        }
     ...
     *
     * @param  array|string|\Micro\Database\Expr $name The table name.
    ...
    public function getPart($part)
    {
        ...
        if (!array_key_exists($part, $this->_parts)) {
            throw new \Micro\Database\Select\Exception("Invalid Select part '$part'");
        }
    ...
    /**
     * Gets the \Micro\Database\Adapter\AdapterAbstract for this
     * particular \Micro\Database\Select object.
     *
     ...
     * @param  array|string|\Micro\Database\Expr $name Table name
     * @param  string $cond Join on this condition
     * @param  array|string $cols The columns to select from the joined table
     * @param  string $schema The database name to specify, if any.
     * @return \Micro\Database\Select This \Micro\Database\Select object
    ...
    protected function _join($type, $name, $cond, $cols, $schema = null)
    {
        if (!in_array($type, self::$_joinTypes) && $type != self::FROM) {
            throw new \Micro\Database\Select\Exception("Invalid join type '$type'");
        }

        if (count($this->_parts[self::UNION])) {
            throw new \Micro\Database\Select\Exception("Invalid use of table with " . self::SQL_UNION);
        }
        ...
        if (!empty($correlationName)) {
            if (array_key_exists($correlationName, $this->_parts[self::FROM])) {
                throw new \Micro\Database\Select\Exception("You cannot define a correlation name '$correlationName' more than once");
            }
    ...
    public function _joinUsing($type, $name, $cond, $cols = '*', $schema = null)
    {
        if (empty($this->_parts[self::FROM])) {
            throw new \Micro\Database\Select\Exception("You can only perform a joinUsing after specifying a FROM table");
        }
    ...
    protected function _tableCols($correlationName, $cols, $afterCorrelationName = null)
    {
        ...
        foreach (array_filter($cols) as $alias => $col) {
            ...
            if (is_string($col)) {
                ...
                // Check for columns that look like functions and convert to \Micro\Database\Expr
                if (preg_match('/\(.*\)/', $col)) {
                    $col = new Expr($col);
    ...
    {
        if (count($this->_parts[self::UNION])) {
            throw new \Micro\Database\Select\Exception("Invalid use of where clause with " . self::SQL_UNION);
        }
    ...
    {
        /*
         * If no table specified, use RDBMS-dependents
     ...
     *
     * @param string $method
     * @param array $args OPTIONAL \Micro\Database\Select query modifier
     * @return \Micro\Database\Select
        ...
        if (preg_match('/^join([a-zA-Z]*?)Using$/', $method, $matches)) {
            ...
            if ($type) {
                ...
                if (!in_array($type, self::$_joinTypes)) {
                    throw new \Micro\Database\Select\Exception("Unrecognized method '$method()'");
                }
                if (in_array($type, array(self::CROSS_JOIN, self::NATURAL_JOIN))) {
                    throw new \Micro\Database\Select\Exception("Cannot perform a joinUsing with method '$method()'");
                }
            } else {
