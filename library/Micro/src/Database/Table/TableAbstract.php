...
abstract class TableAbstract
{
    ...
    /**
     * Default \Micro\Database\Adapter\AdapterAbstract object.
     *
     * @var \Micro\Database\Adapter\AdapterAbstract
     */
     ...
     *
     * @var \Micro\Cache\Core
     */
    protected static $_defaultMetadataCache = null;

    /**
     * \Micro\Database\Adapter\AdapterAbstract object.
     *
     * @var \Micro\Database\Adapter\AdapterAbstract
     */
    protected $_db;
    ...
    /**
     * The table column names derived from \Micro\Database\Adapter\AdapterAbstract::describeTable().
     *
     * @var array
     */
    protected $_cols;
    ...
    protected $_useSharedMetadata = true;

    protected static $_metadataExt = array();
     ...
     * @var string
     */
    protected $_rowsetClass = 'Micro\Database\Table\Rowset';
     ...
     * extend \Micro\Database\Table\TableAbstract.
     *
     * @var array
     */
     ...
     * @return \Micro\Database\Table\TableAbstract Provides a fluent interface
     */
    ...
    /**
     * Sets the default \Micro\Database\Adapter\AdapterAbstract for all \Micro\Database\Table\TableAbstract objects.
     *
    ...
    /**
     * Gets the default \Micro\Database\Adapter\AdapterAbstract for all \Micro\Database\Table\TableAbstract objects.
     *
     * @return \Micro\Database\Adapter\AdapterAbstract or null
     */
    ...
    /**
     * Gets the \Micro\Database\Adapter\AdapterAbstract for this particular \Micro\Database\TableAbstract object.
     *
    ...
    /**
     * Sets the default metadata cache for information returned by \Micro\Database\Adapter\AdapterAbstract::describeTable().
     *
    ...
    /**
     * Gets the default metadata cache for information returned by \Micro\Database\Adapter\AdapterAbstract::describeTable().
     *
     * @return \Micro\Cache\Core or null
     */
    ...
    /**
     * Sets the metadata cache for information returned by \Micro\Database\Adapter\AdapterAbstract::describeTable().
     *
    ...
    /**
     * Gets the metadata cache for information returned by \Micro\Database\Adapter\AdapterAbstract::describeTable().
     *
     * @return \Micro\Cache\Core or null
     */
    ...
    protected static function _setupMetadataCache($metadataCache)
    {
        ...
        if (!$metadataCache instanceof Cache\Core) {
            throw new Exception('Argument must be of type \Micro\Cache\Core, or a Registry key where a \Micro\Cache\Core object is stored');
        }
    ...
    {
        if ($this->metadataCacheInClass() && !empty($this->_metadata)) {
            return true;
        }
        ...
        if (null !== $this->_metadataCache) {
            ...
            // Define the cache identifier where the metadata are saved
            $cacheId = 'DB_' . $this->_name . '_' . md5( // port:host/dbname:schema.table (based on availabilty)
                    $port . $host . '/'. $dbConfig['dbname'] . ':'
                  . $this->_schema. '.' . $this->_name
            );
        }

        if ($this->_useSharedMetadata && isset(self::$_metadataExt[$cacheId])) {
            $this->_metadata = self::$_metadataExt[$cacheId];
            return true;
        }
        ...
        $this->_metadata = self::$_metadataExt[$cacheId] = $metadata;
    ...
    protected function _setupPrimaryKey()
    {
        if (!$this->_primary) {
            ...
            if (empty($this->_primary)) {
                throw new Exception("A table must have a primary key, but none was found for table '{$this->_name}'");
            }
        ...
        */
        if ($this->_sequence === true && $this->_db instanceof Pgsql) {
    ...
    /**
     * Returns an instance of a \Micro\Database\Select object.
     *
    ...
    public function insert(array $data)
    {
        ...
        /**
         * \Micro\Database\Table\TableAbstract assumes that if you have a compound primary key
     ...
     * Fetches all rows.
     *
     * Honors the \Micro\Database\Adapter\AdapterAbstract fetch mode.
     *
     * @param string|array|Select $where  OPTIONAL An SQL WHERE clause or Select object.
     * @param string|array                      $order  OPTIONAL An SQL ORDER clause.
     * @param int                               $count  OPTIONAL An SQL LIMIT count.
     * @param int                               $offset OPTIONAL An SQL LIMIT offset.
     * @return \Micro\Database\Table\Rowset\RowsetAbstract The row results per the \Micro\Database\Adapter\AdapterAbstract fetch mode.
     */
    ...
    /**
     * Fetches one row in an object of type \Micro\Database\Table\Row\RowAbstract,
     * or returns null if no row matches the specified criteria.
     *
     * @param string|array|Select $where  OPTIONAL An SQL WHERE clause or Select object.
     * @param string|array                      $order  OPTIONAL An SQL ORDER clause.
     * @param int                               $offset OPTIONAL An SQL OFFSET value.
     * @return \Micro\Database\Table\Row\RowAbstract|null The row results per the
     * \Micro\Database\Adapter\AdapterAbstract fetch mode, or null if no row found.
     */
    public function fetchRow($where = null, $order = null, $offset = null)
    {
        if (!($where instanceof Select)) {
            ...
            $select = $where->limit(1, $where->getPart(Select::LIMIT_OFFSET));
        }
        ...
        if (!class_exists($rowClass)) {
            throw new Exception("Class '{$rowClass}' does not exists");
        }
