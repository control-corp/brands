...
class SessionNamespace extends SessionAbstract implements \IteratorAggregate
{
    ...
    /**
     * Namespace - which namespace this instance of micro-session is saving-to/getting-from
     *
     * @var string
     */
    ...
    {
        if ($namespace === '') {
            throw new Exception('Session namespace must be a non-empty string.');
        }

        if ($namespace[0] == "_") {
            throw new Exception('Session namespace must not start with an underscore.');
        }

        if (preg_match('#(^[0-9])#i', $namespace[0])) {
            throw new Exception('Session namespace must not start with a number.');
        }

        if (isset(self::$_singleInstances[$namespace])) {
            throw new Exception("A session namespace object already exists for this namespace ('$namespace'), and no additional accessors (session namespace objects) for this namespace are permitted.");
        }
        ...
        if (self::$_readable === false) {
            throw new Exception(self::_THROW_NOT_READABLE_MSG);
        }
        ...
        if (parent::$_writable === true) {
            if (isset($_SESSION['__MICRO'][$namespace])) {

                // Expire Namespace by Namespace Hop (ENNH)
                if (isset($_SESSION['__MICRO'][$namespace]['ENNH'])) {
                    $_SESSION['__MICRO'][$namespace]['ENNH']--;

                    if ($_SESSION['__MICRO'][$namespace]['ENNH'] === 0) {
                ...
                if (isset($_SESSION['__MICRO'][$namespace]['ENVNH'])) {
                    foreach ($_SESSION['__MICRO'][$namespace]['ENVNH'] as $variable => $hops) {
                        $_SESSION['__MICRO'][$namespace]['ENVNH'][$variable]--;

                        if ($_SESSION['__MICRO'][$namespace]['ENVNH'][$variable] === 0) {
                            ...
                            unset($_SESSION['__MICRO'][$namespace]['ENVNH'][$variable]);
                        }
                    }
                    if(empty($_SESSION['__MICRO'][$namespace]['ENVNH'])) {
                        unset($_SESSION['__MICRO'][$namespace]['ENVNH']);
                    }
                }
            }

            if (empty($_SESSION['__MICRO'][$namespace])) {
                unset($_SESSION['__MICRO'][$namespace]);
            }

            if (empty($_SESSION['__MICRO'])) {
    ...
    public function __set($name, $value)
    {
        if (isset(self::$_namespaceLocks[$this->_namespace])) {
            throw new Exception('This session/namespace has been marked as read-only.');
        }
        ...
        if (parent::$_writable === false) {
            throw new Exception(parent::_THROW_NOT_WRITABLE_MSG);
        }
    ...
    public function applySet($callback)
    {
        ...
        if (!is_array($result)) {
            throw new Exception('Result must be an array. Got: ' . gettype($result));
        }
    ...
    public function setExpirationSeconds($seconds, $variables = null)
    {
        ...
        if ($variables === null) {
            ...
            $_SESSION['__MICRO'][$this->_namespace]['ENT'] = time() + $seconds;
            ...
            foreach ($variables as $variable) {
                if (!empty($variable)) {
                    $_SESSION['__MICRO'][$this->_namespace]['ENVT'][$variable] = time() + $seconds;
                }
            }
        }
    }
    ...
    public function setExpirationHops($hops, $variables = null, $hopCountOnUsageOnly = false)
    {
        ...
        if ($hops <= 0) {
            throw new Exception('Hops must be positive number.');
        }
        ...
        if ($variables === null) {
            ...
            if ($hopCountOnUsageOnly === false) {
                $_SESSION['__MICRO'][$this->_namespace]['ENGH'] = $hops;
            } else {
                $_SESSION['__MICRO'][$this->_namespace]['ENNH'] = $hops;
            }
            ...
            foreach ($variables as $variable) {
                if (!empty($variable)) {
                    if ($hopCountOnUsageOnly === false) {
                        $_SESSION['__MICRO'][$this->_namespace]['ENVGH'][$variable] = $hops;
                    } else {
                        $_SESSION['__MICRO'][$this->_namespace]['ENVNH'][$variable] = $hops;
                    }
                }
            }
        }
