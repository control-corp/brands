...
abstract class SessionAbstract
{
     ...
     * but current \Micro\Session\Session has been marked as read-only.
     */
    const _THROW_NOT_WRITABLE_MSG = 'Micro\Session\Session is currently marked as read-only.';

    /**
     * Error message thrown when an action requires reading session data,
     * but current \Micro\Session\Session is not marked as readable.
     */
    const _THROW_NOT_READABLE_MSG = 'Micro\Session\Session is not marked as readable.';
    ...
    protected static function _namespaceUnset($namespace, $name = null)
    {
        if (self::$_writable === false) {
            throw new Exception(self::_THROW_NOT_WRITABLE_MSG);
        }
