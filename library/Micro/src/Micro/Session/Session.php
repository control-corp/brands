...
use Micro\Session\Validator\ValidatorInterface;
use Micro\Session\SaveHandler\SaveHandlerInterface;

class Session extends SessionAbstract
{
    /**
     * Whether or not \Micro\Session\Session is being used with unit tests
     *
     * @internal
     * @var bool
     */
     ...
     *
     * @var bool This could also be a combiniation of error codes to catch
     */
     ...
     *
     * @var \Micro\Session\SaveHandler\SaveHandlerInterface
     */
    ...
    public static function setOptions(array $userOptions = array())
    {
        ...
        foreach ($userOptions as $userOptionName => $userOptionValue) {
            ...
            elseif (isset(self::$_localOptions[$userOptionName])) {
                ...
                throw new Exception("Unknown option: $userOptionName = $userOptionValue");
            }
        }
    }
    ...
    /**
     * @param \Micro\Session\SaveHandler\SaveHandlerInterface $saveHandler
     */
    public static function setSaveHandler(SaveHandlerInterface $saveHandler)
    {
     ...
     *
     * @return \Micro\Session\SaveHandler\SaveHandlerInterface
     */
    ...
    public static function regenerateId()
    {
        if (!self::$_unitTestEnabled && headers_sent($filename, $linenum)) {
            throw new Exception("You must call " . __CLASS__ . '::' . __FUNCTION__ .
    ...
    public static function start($options = false)
    {
        ...
        if (!self::$_unitTestEnabled && headers_sent($filename, $linenum)) {
            throw new Exception("Session must be started before any output has been sent to the browser;"
    ...
    protected static function _checkId($id)
    {
        ...
        if ($saveHandler == 'cluster') { // Micro Server SC, validate only after last dash
            $dashPos = strrpos($id, '-');
            if ($dashPos) {
    ...
    private static function _processStartupMetadataGlobal()
    {
        ...
        if (isset($_SESSION['__MICRO'])) {

            // expire globally expired values
            foreach ($_SESSION['__MICRO'] as $namespace => $namespace_metadata) {

                // Expire Namespace by Time (ENT)
                if (isset($namespace_metadata['ENT']) && ($namespace_metadata['ENT'] > 0) && (time() > $namespace_metadata['ENT']) ) {
                    unset($_SESSION[$namespace]);
                    unset($_SESSION['__MICRO'][$namespace]);
                }

                // Expire Namespace by Global Hop (ENGH) if it wasnt expired above
                if (isset($_SESSION['__MICRO'][$namespace]) && isset($namespace_metadata['ENGH']) && $namespace_metadata['ENGH'] >= 1) {

                    $_SESSION['__MICRO'][$namespace]['ENGH']--;

                    if ($_SESSION['__MICRO'][$namespace]['ENGH'] === 0) {
                ...
                if (isset($namespace_metadata['ENVT'])) {
                    foreach ($namespace_metadata['ENVT'] as $variable => $time) {
                        if (time() > $time) {
                            ...
                            unset($_SESSION['__MICRO'][$namespace]['ENVT'][$variable]);
                        }
                    }
                    if (empty($_SESSION['__MICRO'][$namespace]['ENVT'])) {
                        unset($_SESSION['__MICRO'][$namespace]['ENVT']);
                    }
                }
                ...
                if (isset($namespace_metadata['ENVGH'])) {
                    foreach ($namespace_metadata['ENVGH'] as $variable => $hops) {
                        $_SESSION['__MICRO'][$namespace]['ENVGH'][$variable]--;

                        if ($_SESSION['__MICRO'][$namespace]['ENVGH'][$variable] === 0) {
                            if (isset($_SESSION[$namespace][$variable])) {
                                parent::$_expiringData[$namespace][$variable] = $_SESSION[$namespace][$variable];
                                unset($_SESSION[$namespace][$variable]);
                            }
                            unset($_SESSION['__MICRO'][$namespace]['ENVGH'][$variable]);
                        }
                    }
                    if (empty($_SESSION['__MICRO'][$namespace]['ENVGH'])) {
                        unset($_SESSION['__MICRO'][$namespace]['ENVGH']);
                    }
                }

                if (isset($namespace) && empty($_SESSION['__MICRO'][$namespace])) {
                    unset($_SESSION['__MICRO'][$namespace]);
                }
            }
        }

        if (isset($_SESSION['__MICRO']) && empty($_SESSION['__MICRO'])) {
            unset($_SESSION['__MICRO']);
        }
    }
     ...
     * has been called during this request by \Micro\Session\Session.
     *
     * @return bool
     */
    ...
    {
        if (!self::$_unitTestEnabled && defined('SID')) {
            throw new Exception('The session has already been started.  The session id must be set first.');
        }
        ...
        if (!is_string($id) || $id === '') {
            throw new Exception('You must provide a non-empty string as a session identifier.');
        }
     ...
     *
     * @param \Micro\Session\Validator\ValidatorInterface $validator
     * @return void
     */
     ...
     *
     * @param bool $readonly - OPTIONAL remove write access (i.e. throw error if \Micro\Session\Session's attempt writes)
     * @return void
     */
    ...
    {
        foreach ($_SESSION['__MICRO']['VALID'] as $validator_name => $valid_data) {
            if (class_exists($validator_name)) {
                $validator = new $validator_name;
                if ($validator instanceof Validator\ValidatorInterface && $validator->validate() === false) {
                    throw new Exception("This session is not valid according to {$validator_name}.");
                }
            }
        }
    }
     ...
     * Deprecated: Use getIterator() in \Micro\Session\SessionNamespace.
     *
     * @param string $namespace
     * @return array
     */
    ...
    public static function getIterator()
    {
        if (parent::$_readable === false) {
            throw new Exception(parent::_THROW_NOT_READABLE_MSG);
        }
