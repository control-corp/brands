...
class Profiler
{
    ...
    /**
     * Array of \Micro\Database\Profiler\Query objects.
     *
     * @var array
     */
     ...
     * @return \Micro\Database\Profiler Provides a fluent interface
     */
     ...
     * the \Micro\Database\Profiler::* constants to only save profiles for that type of
     ...
     *
     * @return integer|null
     * @see    \Micro\Database\Profiler::setFilterQueryType()
     */
     ...
     *
     * @param  \Micro\Database\Profiler\Query $query
     * @return integer or null
     */
    public function queryClone(\Micro\Database\Profiler\Query $query)
    {
    ...
    /**
     * Starts a query.  Creates a new query profile object (\Micro\Database\Profiler\Query)
     ...
     * @param  integer $queryType   OPTIONAL Type of query, one of the \Micro\Database\Profiler::* constants
     * @return integer|null
     */
    public function queryStart($queryText, $queryType = null)
    {
        ...
        $this->_queryProfiles[] = new \Micro\Database\Profiler\Query($queryText, $queryType);
     ...
     * @throws \Micro\Database\Profiler\Exception
     * @return string   Inform that a query is stored or ignored.
     */
    public function queryEnd($queryId)
    {
        // Don't do anything if the \Micro\Database\Profiler is not enabled.
        if (!$this->_enabled) {
            return self::IGNORED;
        }
        ...
        if (!isset($this->_queryProfiles[$queryId])) {
            throw new \Micro\Database\Profiler\Exception("Profiler has no query with handle '$queryId'.");
        }
     ...
     * by queryStart() and it will return a \Micro\Database\Profiler\Query object.
     *
     * @param  integer $queryId
     * @throws \Micro\Database\Profiler\Exception
     * @return \Micro\Database\Profiler\Query
     */
    public function getQueryProfile($queryId)
    {
        if (!array_key_exists($queryId, $this->_queryProfiles)) {
            throw new \Micro\Database\Profiler\Exception("Query handle '$queryId' not found in profiler log.");
        }
    ...
    /**
     * Get an array of query profiles (\Micro\Database\Profiler\Query objects).  If $queryType
     * is set to one of the \Micro\Database\Profiler::* constants then only queries of that
     ...
     * one or more of the \Micro\Database\Profiler::* constants, the elapsed time will be calculated
     ...
     * be counted.  If $queryType is set to one of the \Micro\Database\Profiler::* constants, only queries of
    ...
    /**
     * Get the \Micro\Database\Profiler\Query object for the last query that was run, regardless if it has
     * ended or not.  If the query has not ended, its end time will be null.  If no queries have
     * been profiled, false is returned.
     *
     * @return \Micro\Database\Profiler\Query|false
     */
