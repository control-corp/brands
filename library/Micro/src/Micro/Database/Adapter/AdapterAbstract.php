...
abstract class AdapterAbstract
{
    ...
    */
     ...
     * @var string
     */
    protected $_defaultStmtClass = 'Micro\Database\Statement';
     ...
     * @var string
     */
    protected $_defaultProfilerClass = 'Micro\Database\Profiler';
     ...
     * Options
     * \Micro\Database\Database::CASE_NATURAL (default)
     * \Micro\Database\Database::CASE_LOWER
     * \Micro\Database\Database::CASE_UPPER
     *
     * @var integer
     */
    protected $_caseFolding = Database::CASE_NATURAL;

    /**
     * Specifies whether the adapter automatically quotes identifiers.
     * If true, most SQL generated by \Micro\Database\Database classes applies
     ...
     * \Micro\Database\Database::INT_TYPE, \Micro\Database\Database::BIGINT_TYPE, or \Micro\Database\Database::FLOAT_TYPE.
     *
     * Values are:
     * 0 = 32-bit integer
     * 1 = 64-bit integer
    ...
    */
    ...
    public function __construct($config)
    {
        ...
        $options = array(
            Database::CASE_FOLDING           => $this->_caseFolding,
            Database::AUTO_QUOTE_IDENTIFIERS => $this->_autoQuoteIdentifiers,
            Database::FETCH_MODE             => $this->_fetchMode,
        );
        ...
        */
        ...
        if (array_key_exists(Database::CASE_FOLDING, $options)) {
            $case = (int) $options[Database::CASE_FOLDING];
            switch ($case) {
                case Database::CASE_LOWER:
                case Database::CASE_UPPER:
                case Database::CASE_NATURAL:
                    $this->_caseFolding = $case;
                    break;
                default:
                    throw new Exception('Case must be one of the following constants: '
                            . '\Micro\Database\Database::CASE_NATURAL, \Micro\Database\Database::CASE_LOWER, \Micro\Database\Database::CASE_UPPER');
            }
        }

        if (array_key_exists(Database::FETCH_MODE, $options)) {
            if (is_string($options[Database::FETCH_MODE])) {
                $constant = 'Micro\Database\Database::FETCH_' . strtoupper($options[Database::FETCH_MODE]);
                if(defined($constant)) {
                    $options[Database::FETCH_MODE] = constant($constant);
                }
            }
            $this->setFetchMode((int) $options[Database::FETCH_MODE]);
        }

        // obtain quoting property if there is one
        if (array_key_exists(Database::AUTO_QUOTE_IDENTIFIERS, $options)) {
            $this->_autoQuoteIdentifiers = (bool) $options[Database::AUTO_QUOTE_IDENTIFIERS];
        }

        // obtain allow serialization property if there is one
        if (array_key_exists(Database::ALLOW_SERIALIZATION, $options)) {
            $this->_allowSerialization = (bool) $options[Database::ALLOW_SERIALIZATION];
        }

        // obtain auto reconnect on unserialize property if there is one
        if (array_key_exists(Database::AUTO_RECONNECT_ON_UNSERIALIZE, $options)) {
            $this->_autoReconnectOnUnserialize = (bool) $options[Database::AUTO_RECONNECT_ON_UNSERIALIZE];
        }

        // create a profiler object
        $profiler = false;
        if (array_key_exists(Database::PROFILER, $this->_config)) {
            $profiler = $this->_config[Database::PROFILER];
            unset($this->_config[Database::PROFILER]);
        }
     ...
     *
     * @param  \Micro\Database\Profiler|array|boolean $profiler
     * @return \Micro\Database\Adapter\AdapterAbstract Provides a fluent interface
     * @throws \Micro\Database\Profiler\Exception if the object instance or class specified
     *         is not \Micro\Database\Profiler or an extension of that class.
     */
    public function setProfiler($profiler)
    {
        ...
        if (($profilerIsObject = is_object($profiler)) !== false) {
            if ($profiler instanceof \Micro\Database\Profiler) {
                $profilerInstance = $profiler;
            } else {
                throw new \Micro\Database\Profiler\Exception('Profiler argument must be an instance of either Profiler');
            }
        }
        ...
        if ($profilerInstance === null) {
            if (!class_exists($profilerClass)) {
                throw new \Micro\Database\Profiler\Exception('Class ' . $profilerClass . ' does not exists');
            }
            $profilerInstance = new $profilerClass();
        }

        if (!$profilerInstance instanceof \Micro\Database\Profiler) {
            throw new \Micro\Database\Profiler\Exception('Class ' . get_class($profilerInstance) . ' does not extend ' . 'Profiler');
        }
     ...
     *
     * @return \Micro\Database\Adapter\AdapterAbstract Fluent interface
     */
     ...
     * @return \Micro\Database\Statement\StatementInterface
     */
    public function query($sql, $bind = array())
    {
        ...
        if ($sql instanceof \Micro\Database\Select) {
            if (empty($bind)) {
                $bind = $sql->getBind();
            }
    ...
    {
        $this->_connect();
        $q = $this->_profiler->queryStart('begin', \Micro\Database\Profiler::TRANSACTION);
    ...
    {
        $this->_connect();
        $q = $this->_profiler->queryStart('commit', \Micro\Database\Profiler::TRANSACTION);
        $this->_commit();
        $this->_profiler->queryEnd($q);
        return $this;
    }
    ...
    public function insert($table, array $bind)
    {
        ...
        foreach ($bind as $col => $val) {
            ...
            if ($val instanceof \Micro\Database\Expr) {
                $vals[] = $val->__toString();
                unset($bind[$col]);
            } else {
    ...
    protected function _whereExpr($where)
    {
        ...
        foreach ($where as $cond => &$term) {
            ...
            if (is_int($cond)) {
                ...
                if ($term instanceof \Micro\Database\Expr) {
    ...
    {
        return new \Micro\Database\Select($this);
    }
     ...
     * the first column contains unique values, or else
     ...
     public function fetchAssoc($sql, $bind = array())
     {
         $stmt = $this->query($sql, $bind);
         $data = array();
         while (($row = $stmt->fetch(Database::FETCH_ASSOC)) !== false) {
     ...
     public function fetchPairs($sql, $bind = array())
     {
         $stmt = $this->query($sql, $bind);
         $data = array();
         while (($row = $stmt->fetch(Database::FETCH_NUM)) !== false) {
             $data[$row[0]] = $row[1];
         }
         return $data;
     }
     ...
     public function quote($value, $type = null)
     {
         ...
         if ($value instanceof \Micro\Database\Select) {
             return '(' . $value->assemble() . ')';
         }

         if ($value instanceof \Micro\Database\Expr) {
         ...
         if ($type !== null && array_key_exists($type = strtoupper($type), $this->_numericDataTypes)) {
             ...
             switch ($this->_numericDataTypes[$type]) {
                 case Database::INT_TYPE: // 32-bit integer
                     $quotedValue = (string) intval($value);
                     break;
                 case Database::BIGINT_TYPE: // 64-bit integer
                     ...
                     if (preg_match('/^(
                        ...
                        break;
                 case Database::FLOAT_TYPE: // float or decimal
     ...
     */
      ...
      *
      * @param string|\Micro\Database\Select $sql SQL query
      * @return \Micro\Database\Statement|\PDOStatement
     */
